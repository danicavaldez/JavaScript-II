...
let allCaps = runners.map(runnersName => runnersName.first_name.toUpperCase());
console.log(allCaps); 

// ==== Challenge 3: Use .filter() ====
// The large shirts won't be available for the event due to an ordering issue.  Get a list of runners with large sized shirts so they can choose a different size. Return an array named largeShirts that contains information about the runners that have a shirt size of L and log the result

let largeShirts = runners.filter( wearsLarge => wearsLarge.shirt_size === "L");
...
ticketPriceTotal.push(runners.reduce((accDonation, runnerName) => accDonation += runnerName.donation, 0));
...
// Problem 1
//Create a new array with last names in alphabetical order.
let runnerSorted = [];

runners.map( people => runnerSorted.push(`${people.last_name}, ${people.first_name}`));

console.log(runnerSorted.sort());
